...
namespace DTPicker
{
    ...
    ///     <ul>
    ///         <li>
    ///             Bug fix: doesn't crash if the cursor is at the end of the textbox with
    ///             no selection, and user presses the left arrow key.
    ///         </li>
    ///         <li>
    ///             Bug fix: when the user clicks one of the punctuation marks in the
    ///             textbox, the field to the right is selected (instead of something weird)
    ///         </li>
    ///         <li>
    ///             Bug fix: user can enter a 4-digit year (in VB, it was cleared after the
    ///             second digit)
    ///         </li>
    ///         <li>
    ///             Bug fix: after typing a single-digit month, day or hour, the left/right
    ///             arrow keys will select the adjacent field correctly.
    ///         </li>
    ///         <li>
    ///             Bug fix: date formats that contain both "dd" and "ddd" are handled
    ///             correctly, e.g. "ddd MM dd, yyyy HH:mm".
    ///         </li>
    ///         <li>
    ///             Bug fix: any custom DateFormat is now permitted, not just formats that
    ...
    ///         <li>
    ///             XAML changed so that the control expands to fill the space it is given,
    ///             because IMO the control looks strange if it changes width as the user is
    ///             typing.
    ///         </li>
    ///         <li>
    ///             After typing a new value for a field, the user can type the appropriate
    ...
    ///         </li>
    ///         <li>
    ///             Free-form editing of the date is now permitted. The user can delete the
    ...
    ///             SelectedDate) when the control loses focus.
    ///         </li>
    ///         <li>
    ///             Added up and down arrows (WPF Adorners) that appear when you select a
    ...
    public partial class DateTimePicker : UserControl
    {
        ...
        public DateTimePicker()
        {
            ...
            Loaded += (s, e) =>
            {
                var adLayer = GetAdornerLayer(DateDisplay);
                if (adLayer != null)
                {
        ...
        {
            var retval = (ParseDateText(false) ?? SelectedDate);
        ...
        private string _inputDateFormat;
        ...
        public static readonly RoutedEvent DateChangedEvent = EventManager.RegisterRoutedEvent("DateChanged",
            RoutingStrategy.Bubble, typeof (RoutedEventHandler), typeof (DateTimePicker));
        ...
        public static readonly RoutedEvent DateFormatChangedEvent = EventManager.RegisterRoutedEvent(
            "DateFormatChanged", RoutingStrategy.Bubble, typeof (RoutedEventHandler), typeof (DateTimePicker));
        ...
        public static readonly DependencyProperty DateFormatProperty = DependencyProperty.Register("DateFormat",
            typeof (string), typeof (DateTimePicker),
            new FrameworkPropertyMetadata("yyyy-MM-dd HH:mm", OnDateFormatChanged));

        public static DependencyProperty MaximumDateProperty = DependencyProperty.Register("MaximumDate",
            typeof (DateTime), typeof (DateTimePicker),
            new FrameworkPropertyMetadata(Convert.ToDateTime("3000-01-01 00:00"), null,
                CoerceMaxDate));

        public static DependencyProperty MinimumDateProperty = DependencyProperty.Register("MinimumDate",
            typeof (DateTime), typeof (DateTimePicker),
            new FrameworkPropertyMetadata(Convert.ToDateTime("1900-01-01 00:00"), null,
                CoerceMinDate));
        ...
        /// <summary>
        ///     true when user is busy editing DateDisplay and the SelectedDate
        ///     becomes different from the date shown on the text box.
        /// </summary>
        public static readonly DependencyProperty DateTextIsWrongProperty =
            DependencyProperty.Register("DateTextIsWrong", typeof (bool), typeof (DateTimePicker),
        ...
        private DateTime? ParseDateText(bool flexible)
        {
            ...
            if (
                !DateTime.TryParseExact(DateDisplay.Text, InputDateFormat(), null, DateTimeStyles.AllowWhiteSpaces,
                    out selectedDate))
        ...
        private void DateTimePicker_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            ...
            switch (e.Key)
            {
                ...
                default:
                    ...
                    if ((e.Key == Key.OemMinus || e.Key == Key.Subtract || e.Key == Key.OemQuestion ||
                         e.Key == Key.Divide) &&
                        (nextChar == '/' || nextChar == '-') ||
        ...
        private static object CoerceDate(DependencyObject d, object value)
        {
            ...
            var current = Convert.ToDateTime(value);
        ...
        public static void OnSelectedDateChanged(DependencyObject obj, DependencyPropertyChangedEventArgs args)
        {
            ...
            {
                var oldDate = me.ParseDateText(true);
                if (oldDate != null)
